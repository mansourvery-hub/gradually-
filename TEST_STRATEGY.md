...
| A-02/A-03 composition & widget purity | convention + review | boundary test covers domain dirs; `test/content_architecture_test.dart` scans `reader/` for content ids + corpus/persistence imports |
| A-09/A-10 content-as-data, order-independent sequencing | architecture test | `test/content_architecture_test.dart` (add-20-stories via data only), `test/selector_golden_test.dart` (input order never matters), `test/end_to_end_flow_test.dart` (full-corpus walk) |
| C-06/C-07/C-08/C-09 content contracts | validator + tests | `tool/validate_curriculum.dart` (manifest parity, offsets, D-06, order floor), `test/content_test.dart`, `test/dynamic_exposure_test.dart` |
...
| D-06 story lexicon purity | regression test | `test/reading_position_test.dart`, `test/compliance_audit_test.dart` (data + runtime corpus), validator |
...
| R-05 ASCII asset names | manifest test | `test/visual_manifest_test.dart` + lexicon asset refs |
...
| P-08 media-optional content | architecture test | `test/content_architecture_test.dart` (media-less stories load, select, round-trip) |
...
| level hook determinism | unit tests | `test/exposure_gate_test.dart` (LEVEL mapping over the data corpus) |

## Verification gates

```text
targeted test      → while iterating a brick
flutter test       → before declaring any task done (156 tests)
