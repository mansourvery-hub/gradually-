...
|---|---|---|---|
| **Bootstrap Target Lexicon Size** | **100 – 150 Hanzi / Words** | Determined backwards from the first 5–10 short stories. Enough to achieve high reading flow on initial stories. Currently: 45 (first batch); expansion is T12. | `[CONFIRMED]` |
...
| **Data-Driven Dynamic Generation** | **100% data-driven** | Exposure sequence is generated dynamically from `bootstrap_target_lexicon.json` without hardcoded code changes. Delivered: units are generated from lexicon data (ADR-007). | `[CONFIRMED]` |
...
| **Story i+1 Readiness (V2 selector)** | **unseen ≤ 6 AND seen ≥ 60%** | A story is offered when it adds at most 6 unencountered words and at least 60% of its vocabulary has been encountered. | `[CONFIRMED]` |
...
- **Full-Screen Presentation:** All visuals (concept art and story illustrations) render full-screen/edge-to-edge — not small inset cards.
- **Asset Generation Stages (confirmed direction):**
  1. *Current (bootstrap placeholder):* Clean geometric/line-art SVGs generated programmatically — sufficient for the learning loop, not final art.
  2. *AI-Generated Final Art (planned):* All concept photos/illustrations for individual hanzi and words will be generated by AI **in a Chinese art style** (specific style directions TBD by editor — a few candidates in mind; this is a human editorial gate). Replaces files in `assets/images/concepts/` via the manifest, no code changes.
  3. *Story Illustrations & Animations (planned):* Full-screen illustrations and lightweight animations for short stories and children's stories will be generated **separately by AI** (different pipeline/style pass from concept art). Replaces files in `assets/images/stories/`.
- **Swap Contract:** The `bootstrap_visuals.json` manifest is the stable interface — AI-generated finals drop in at the same paths without touching reader code. Concept-visual references live in the lexicon data (`asset` field per word); story visuals live in each story JSON (`visualAsset` per section).
